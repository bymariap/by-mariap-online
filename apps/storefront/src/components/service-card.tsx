...
export function ServiceCard({ service }: { service: ServiceDTO }) {
  return (
    <div className="bg-white rounded-md p-6 flex flex-col gap-4" style={{ boxShadow: '0 20px 40px rgba(48,51,46,0.05)' }}>
      <div>
        <h3 className="font-heading text-lg font-semibold text-foreground">{service.name}</h3>
        {service.description && (
          <p className="mt-1 text-sm font-body text-muted-foreground line-clamp-2">{service.description}</p>
        )}
      </div>
